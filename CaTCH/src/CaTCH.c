...
SEXP catch(SEXP input)
{
	...
	insulation = (float **) calloc(ND+1,sizeof(float *));
        for (i=0;i<ND+1;i++) insulation[i] = (float *) calloc(size,sizeof(float));

	for(i=0;i<nrow;i++){
		if(REAL(input)[i+2*nrow]!=-1)	mat[(int)REAL(input)[i+0*nrow]][(int)REAL(input)[i+1*nrow]]=(unsigned short) REAL(input)[i+2*nrow];
	...
	PROTECT(out = Rf_allocMatrix(REALSXP, tot,4));
        ...
        for (i=1;i<=ND;i++)
			...
			REAL(out)[appo+tot*3] = insulation[i][j];
                        appo++;
                }
